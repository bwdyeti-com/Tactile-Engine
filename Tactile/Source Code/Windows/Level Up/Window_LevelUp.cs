...
namespace Tactile
{
    internal class Window_LevelUp : Sprite
    {
        ...
        private TextSprite Class_Name, Lv_Label;
        private RightAdjustedText Level;
        private List<TextSprite> Stat_Labels = new List<TextSprite>();
        private List<RightAdjustedText> Stats = new List<RightAdjustedText>();
        ...
        {
            texture = Content.Load<Texture2D>(@"Graphics/Windowskins/Level_Up_Window");
            ...
            Stat_Gain_Textures.Add(Content.Load<Texture2D>(@"Graphics/Fonts/" + Config.PROMOTION_STAT_FONT));
            Stat_Gain_Textures.Add(Content.Load<Texture2D>(@"Graphics/Fonts/" + Config.LEVEL_STAT_FONT));
            ...
            // Class Name
            Class_Name = new TextSprite();
            Class_Name.SetFont(Config.UI_FONT, Global.Content, "White");
            Class_Name.offset = new Vector2(0, 8);
            // Lv Label
            Lv_Label = new TextSprite();
            Lv_Label.SetFont(Config.UI_FONT, Global.Content, "Yellow");
            Lv_Label.text = "Lv";
            Lv_Label.offset = new Vector2(-(64 + 6), 8);
            // Level
            Level = new RightAdjustedText();
        ...
        protected virtual void refresh_stat_label_color(int i)
        {
            ...
            {
                if (Global.ActorConfig.StatLabelColoring == TactileLibrary.Config.StatLabelColoring.Averages)
                {
                    Stat_Labels[i].tint = actor.stat_color(
                        (Stat_Labels)i,
                        TactileLibrary.Config.StatLabelColoring.Averages,
                        actor.needed_levels,
                        2f);
                }
                else
