...
namespace Tactile
{
    ...
    class Window_Message : Stereoscopic_Graphic_Object
    {
        ...
        internal static Font_Data FontData { get { return Font_Data.Data[Config.CONVO_FONT]; } }
        #endregion
...
function normally. Suggested value is -3.",
        ...
        protected List<int> get_text_widths(List<string> text_list)
        {
            ...
            for (int index = 0; index < text_array.Count; index++)
            {
                string str = text_array[index];
                        ...
                        switch (key)
                        {
                            ...
                            case "F":
                                ...
                                if (valid_face_speaker(face_id))
                                {
                                    ...
                                    names[face_id] = name.Split(Global.ActorConfig.ActorNameDelimiter)[0];
                                }
        ...
        {
            int new_width = Font_Data.text_width(copy_text2, Config.CONVO_FONT) + offset;
            width = Math.Max(new_width, width);
        }
        ...
        protected int new_box(int? speaker, ref int width, ref int stop_width)
        {
            ...
            if (width % 8 != 0)
                width += 8 - (width % 8);
            // Minimum width of 16 for the speaker arrow
        ...
        private Maybe<bool> process_next_character()
        {
            ...
            Text_Loc.X += Font_Data.text_width("" + current_character, Config.CONVO_FONT);
                ...
                if (text_command.StartupCommand)
                {
                    ...
                    switch (key)
                    {
                        ...
                        case "F":
                            ...
                            {
                                Names[id] = filename.Split(Global.ActorConfig.ActorNameDelimiter)[0];
        ...
        private static string FixGenericName(string baseName)
        {
            ...
            string[] nameAry = baseName.Split(Global.ActorConfig.BuildNameDelimiter);
            int baseBuild;
