...
using Tactile.Calculations.LevelUp;
...
namespace Tactile
{
    ...
    internal partial class Game_Actor
    {
        ...
        public string name
        {
            get
            {
                return this.name_full.Split(Global.ActorConfig.ActorNameDelimiter)[0];
            }
            set
            {
                Name = value;
        ...
        internal static string ConstructGenericFaceName(string name, int gender, int build)
        {
            return string.Format("{0}{1}{2}{3}",
                name,
                (gender % 2 == 0 ? "M" : "F"),
                Global.ActorConfig.BuildNameDelimiter,
                build);
        }
            ...
            set
            {
                ...
                //{
                    ...
                    if (Global.data_classes.ContainsKey(value))
                    {
                        ...
                        if (Global.ActorConfig.ActorGainedHpHeal)
        ...
        public int exp
        {
            ...
            set
            {
                ...
                if (Exp >= Global.ActorConfig.ExpToLvl)
                {
                    // Level up promotion needed
                    if (!can_level(false) && can_level())
                    {
                        Needs_Promotion = true;
                        Exp = Exp % Global.ActorConfig.ExpToLvl;
                    }
                    else if (can_level(false))
                    {
                        while (Exp >= Global.ActorConfig.ExpToLvl)
                        {
                            Needed_Levels++;
                            Exp -= Global.ActorConfig.ExpToLvl;
                        }
                        if (Instant_Level)
                        {
            ...
            bool fixed_level = false, int[] growths = null, int[] wexp = null)
        {
            ...
            if (fixed_level)
            {
                fixed_lvl_up(exp_gain / Global.ActorConfig.ExpToLvl);
                this.exp = exp_gain % Global.ActorConfig.ExpToLvl;
            }
            else
            {
                int total_levels = (exp_gain / Global.ActorConfig.ExpToLvl);
                exp_gain -= total_levels * Global.ActorConfig.ExpToLvl;
                ...
                {
                    Instant_Level = true;
                    this.exp = Global.ActorConfig.ExpToLvl;
                    level_down();
                }
                this.exp = exp_gain;
            }
            ...
            if (Global.ActorConfig.GenericAutoWexp)
            {
        ...
        private void set_generic_growths(int level, int exp, int prepromote_levels, int[] growths)
        {
            ...
            else
            {
                ...
                if (Global.ActorConfig.GenericActorRandomAffinities)
        ...
        private int generic_exp(int level, int prepromote_levels)
        {
            ...
            if (Global.ActorConfig.ResetLevelOnPromotion)
                levels -= (this.tier - Global.ActorConfig.LowestTier);
            int result = levels * Global.ActorConfig.ExpToLvl;
            return result;
        }

        private int get_generic_random_level_count(int levels)
        {
            float random_percent = Global.ActorConfig.GenericRandomLevelPercent;
            int random_max = Global.ActorConfig.GenericRandomLevelMax;
#if DEBUG
        ...
        internal int gained_levels()
        {
            ...
            if (Global.ActorConfig.ResetLevelOnPromotion)
            {
                ...
                if (this.is_generic_actor)
                    gainedTiers = this.tier - Global.ActorConfig.LowestTier;
                else
                {
        ...
        internal int full_base_level
        {
            get
            {
                ...
                {
                    var baseClass = Global.data_classes[Data.ClassId];
                    return Global.ActorConfig.LevelsBeforeTier(baseClass.Tier) + Data.Level;
                }
            }
        }
        ...
        private int GainedLevelsForAverages()
        {
            ...
            if (Global.ActorConfig.ResetLevelOnPromotion && !this.is_generic_actor)
            {
                var baseClass = Global.data_classes[Data.ClassId];
                for (int i = baseClass.Tier; i < this.tier; i++)
                {
                    // Use assumed promotion level
                    int avgPromotionLevel = Global.ActorConfig.AveragePromotionLevel(i);
                    // Use the starting level for the base tier,
                    // if can promote at base
                    if (i == baseClass.Tier)
                        avgPromotionLevel = Math.Max(Data.Level, avgPromotionLevel);

                    int levelCap = Global.ActorConfig.LevelCap(i);
        ...
        private static Vector3 StatBaseHue
        {
            get
            {
                return new Vector3(1f, 30 / 31f, 17 / 31f) * 1f;
            }
        }
        internal static Color StatLabelBaseTint { get { return new Color(StatBaseHue); } }

        internal Color stat_color(Stat_Labels stat_label)
        {
            return stat_color(stat_label, Global.ActorConfig.StatLabelColoring);
        }
        internal Color stat_color(
            Stat_Labels stat_label,
            StatLabelColoring labelColoring,
            int levels = 0,
            float multiplier = 1f)
        {
            Vector3 hue = StatBaseHue;

            switch (labelColoring)
            {
                case StatLabelColoring.Averages:
                    ...
                    hue.X = MathHelper.Clamp(hue.X - (stat_quality * 1.2f), 0f, 1f);
                    hue.Y = MathHelper.Clamp(hue.Y + stat_quality, 0f, 1f);
                    if (stat_quality > 0)
                        hue = Vector3.Lerp(hue, Vector3.One, Math.Min(stat_quality * 1.2f, 1) * 0.7f);
                    return new Color(hue);
                case StatLabelColoring.Growths:
                    ...
                    if ((int)stat_label < this.Data.Growths.Count)
                    {
                        ...
                        growth = (int)MathHelper.Clamp(
                            growth,
                            Global.ActorConfig.GrowthAverageColorMin,
                            Global.ActorConfig.GrowthAverageColorMax);
                        //Sets the function to be within the range provided.
                        if (growth == Global.ActorConfig.GrowthAverageColorMed)
                        {
                            return new Color(hue);
                        }
                        else if(growth < Global.ActorConfig.GrowthAverageColorMed)
                        {
                            growth -= Global.ActorConfig.GrowthAverageColorMin;
                            int mid = Global.ActorConfig.GrowthAverageColorMed - Global.ActorConfig.GrowthAverageColorMin;
                            float value = (growth * multiplier) / mid;
                            float green = MathHelper.Clamp(0.25f + 0.75f * value, 0f, 1f);
                            hue.Y = MathHelper.Clamp(hue.Y * green, 0f, 1f);
                            return new Color(hue);
                        }
                        else
                        {
                            growth -= Global.ActorConfig.GrowthAverageColorMed;
                            int max = Global.ActorConfig.GrowthAverageColorMax - Global.ActorConfig.GrowthAverageColorMed;
                            float value = (growth * multiplier) / max;
                            float red = MathHelper.Clamp(1f - 0.75f * value, 0f, 1f);
                            hue.X = MathHelper.Clamp(hue.X * red, 0f, 1f);
                            return new Color(hue);
                        }
                    }
                    return new Color(hue);
                case StatLabelColoring.None:
                default:
        ...
        public bool promotion_level()
        {
            ...
            if (Global.ActorConfig.ResetLevelOnPromotion)
                ...
                level = this.full_level -
                    Global.ActorConfig.LevelsBeforeTier(this.tier);

            return level >= Global.ActorConfig.PromotionLevel(this.tier);
        }

        //@Yeti: replace with system rng call or something
        ...
        public bool can_level(bool promotion_check)
        {
            ...
            if (Config.LEVEL_UP_PROMOTION.Contains(tier) &&
                    (Needs_Promotion ? Exp < Global.ActorConfig.ExpToLvl : true) &&
        ...
        {
            // This should be modified to work if exp gain is greater than Config.EXP_TO_LVL //Debug
            if (Exp + exp_gain >= Global.ActorConfig.ExpToLvl)
                if (!can_level(false) && can_level())
                {
                    return true;
                }
        ...
        {
            return Global.ActorConfig.LevelCap(this.tier);
        }
        
        public int exp_loss_possible()
        {
            return (Level - 1) * Global.ActorConfig.ExpToLvl + exp;
        }
        ...
        public int exp_gain_possible()
        {
            ...
            return levels * Global.ActorConfig.ExpToLvl - exp;
        }
        ...
        private bool should_use_semifixed_level
        {
            get
            {
                if (Global.ActorConfig.SemifixedLevelsAtPreparations)
        ...
        public int get_cap(Stat_Labels i)
        {
            switch (i)
            {
                case Stat_Labels.Lck:
                    return Global.ActorConfig.LuckCap;
        ...
        internal static float GetStatValue(int i)
        {
            switch (i)
            {
                case (int)Stat_Labels.Hp:
                    return Global.ActorConfig.HpValue;
                default:
                    return 1.0f;
            }
        }
            ...
            if (result.Count > 0)
            {
                for (int i = 1; i < Global.weapon_types.Count; i++)
                {
                    ...
                    if (result[i - 1] != 0)
                    {
                        ...
                        // Gain wexp
                        if (!Config.SINGLE_ANIMA_PROMOTION || !TactileLibrary.Data_Weapon.ANIMA_TYPES.Contains(i))
                        {
                        ...
                        else
                        {
                            ...
                            bool all_anima = true;
                            foreach (int anima in TactileLibrary.Data_Weapon.ANIMA_TYPES)
                            {
        ...
        public int move_speed
        {
            ...
            {
                // Armor
                if (actor_class.Movement_Type == TactileLibrary.MovementTypes.Armor &&
        ...
        public int max_weapon_level(WeaponType type, bool absolute = false)
        {
            ...
            if (type.Key - 1 >= this.actor_class.Max_WLvl.Count)
            {
#if DEBUG
                throw new ArgumentOutOfRangeException("type", string.Format(
                    "Class {0} doesn't have a weapon rank for {1}.\n" +
                    "Class data likely needs to be edited and re-saved.",
                    this.actor_class.Name, type.Name));
#endif
                return 0;
            }
            int rank = this.actor_class.Max_WLvl[type.Key - 1];
            rank = max_weapon_level_skill(type, rank);
            if (!absolute && Global.ActorConfig.OneSRank &&
        ...
        {
            return Global.ActorConfig.SRankBonus;
        }
        #endregion
        ...
        public void organize_items()
        {
            if (Equipped != 0)
            {
                ...
                Items = temp_items;
                while (Items.Count < Global.ActorConfig.NumItems)
                    Items.Add(new Item_Data());
                Equipped = 1;
            }
            else
        ...
        /// </summary>
        public bool is_full_items { get { return num_items >= Global.ActorConfig.NumItems; } }
        /// <summary>
        /// Returns true if the inventory has more than the max number of items
        /// </summary>
        public bool too_many_items { get { return num_items > Global.ActorConfig.NumItems; } }
        /// <summary>
        ...
        internal void item_effect(Data_Item item, int target_index)
        {
            ...
            if (item.can_repair && target_index >= 0)
                ...
                {
                    int repairUses = Items[target_index].RepairAmount(item);

                    Items[target_index].repair(false, repairUses);
                }
        }
        ...
        public WeaponType valid_weapon_type_of(Data_Weapon weapon)
        {
            ...
            if (Global.ActorConfig.ParentWeaponTypeAllowsChild)
            {
                // If this type can't already be used as its own type
                if ((int)weapon.Rank > get_weapon_level(weapon_type))
        ...
        public bool is_equippable(List<Item_Data> items, int index)
        {
            ...
            var weapon = item_data.to_weapon;
            if (index == Siege_Engine.SiegeInventoryIndex)
        ...
        public bool class_equippable(Data_Weapon weapon)
        {
            ...
            // Check if the weapon's type is a parent of a type this actor has
            if (Global.ActorConfig.ChildWeaponTypeAllowsParent && !valid_rank)
            {
                foreach (var child_type in Global.weapon_types)
                    if (child_type.type_and_parents(Global.weapon_types)
                            .Skip(1)
            ...
            // Check if the weapon's type is a child of a type this actor has
            if (Global.ActorConfig.ParentWeaponTypeAllowsChild && !valid_rank)
            {
                foreach (var parent_type in weapon_type.type_and_parents(Global.weapon_types).Skip(1))
        ...
        public bool NeedsPromotionMenu
        {
            get
            {
                return Global.ActorConfig.PromotionSelectionMenu &&
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                if (item_data.is_weapon)
                {
                    ...
                    if (!weapon.is_staff())
                    {
                        ...
                        if (!can_equip)
                            can_equip = i == Siege_Engine.SiegeInventoryIndex &&
        ...
        {
            return (int)(Global.ActorConfig.LowHealthRate * this.maxhp);
        }
