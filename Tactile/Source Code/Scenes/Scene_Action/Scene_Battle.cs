    ...
    class Scene_Battle : Scene_Action
    {
        ...
        protected void initialize_weather_condition()
        {
            if (Weather_Visible == BattleWeathers.Visible)
            {
                ...
                if (Global.data_terrains[tag].Move_Costs[(int)Weather_Types.Rain][(int)TactileLibrary.MovementTypes.Flying] == 1)
        ...
        protected override void update_shake()
        {
            ...
            if (Shake.Count > 0)
            {
                int shakeAttacker = Math.Abs(Shake_Attacker);
                Vector2 shake = Shake.pop();
                if (shakeAttacker == 2 ^ !Reverse)
                    shake.X *= -1;
                // Melee range or all shake
                if (Distance == 1 || Shake_Attacker < 0)
                {
                    Layer_2_Shake += shake;
                ...
                {
                    if (Mini_Shaking ^ (shakeAttacker != 1) ^ Reverse) // fiddle with until it works for all situations//Debug
                    ...
                    case Battle_Actions.Death_Quote:
                        ...
                        if (hit_over(magic_attack, battler_1_sprite))
                        {
                            ...
                            if (dying_unit.battler.is_player_team) //Multi
                            {
                                ...
                                {
                                    Global.Audio.PlayBattleTheme(Global.BgmConfig.GameOverTheme);
                                }
                                else
                                {
                                    Global.Audio.PlayBattleTheme(Global.BgmConfig.AllyDeathTheme);
                                }
                            }
                        }
                        break;
