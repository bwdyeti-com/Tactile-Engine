...
namespace Tactile
{
    ...
    class Combat
    {
            ...
            if (result.hit)
            {
                ...
                if (weapon.Cursed())
                    result.backfire = Global.game_system.roll_rng(
                        Global.ActorConfig.LuckCap - battler_1.stat(Stat_Labels.Lck) +
        ...
        public static void map_attack(Game_Unit battler_1, Combat_Map_Object target, bool is_target_unit, Combat_Round_Data data, TactileLibrary.Data_Weapon weapon)
        {
        ...
        public static int exp(int diff, bool boss = false, bool kill = false)
        {
            ...
            return Math.Min(exp_gain, Global.ActorConfig.ExpToLvl);
        }
