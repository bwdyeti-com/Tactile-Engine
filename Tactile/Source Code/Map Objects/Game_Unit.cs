...
namespace Tactile
{
    ...
    internal partial class Game_Unit : Combat_Map_Object
    {
        ...
        public List<Item_Data> items
        {
            ...
            {
                //Debug
                //List<Item_Data> items = actor.items.GetRange(0, Global.ActorConfig.NUM_ITEMS);
        ...
        public bool is_weapon_broke(int uses)
        {
            ...
            {
                if (Siege_Engine.SiegeInventoryIndex >= this.items.Count)
                    return true;
                item = this.items[Siege_Engine.SiegeInventoryIndex];
            }
            else
        ...
        public int dance_exp()
        {
            ...
            if (!is_player_team)
                exp_gain = Math.Min(exp_gain, Global.ActorConfig.ExpToLvl -
                    (this.actor.exp + 1));
        ...
        public IEnumerable<Siege_Engine> sieges_in_range_of_target(IEnumerable<Siege_Engine> sieges, Vector2 targetLoc)
        {
            return sieges.Where(x =>
            {
                ...
                if (move_distance.IsSomething)
                {
                    ...
                    int min = min_range(Siege_Engine.SiegeInventoryIndex);
        ...
        {
            return Math.Min(
                expGain, Global.ActorConfig.ExpPerEnemy - this.exp_given);
        }
        ...
        public bool allowed_to_gain_exp()
        {
            ...
            if (!is_ally)
            {
                ...
                if (!Global.ActorConfig.CitizensGainExp ||
        ...
        public void use_item(int index)
        {
            ...
            if (this.items[index].out_of_uses)
                if (index < Global.ActorConfig.NumItems) // Ensures this is working right for siege
        ...
        public void use_chest_key()
        {
            ...
            return;
            // Why does this use < Global.ActorConfig.NUM_ITEMS, but check the items list which includes siege? //Debug
                        ...
                        !Global.data_weapons[siege.item.Id].is_staff())
                    {
                        ...
                        if (move_distance.IsSomething)
                        {
                            ...
                            result = check_range(
                                ...
                                this.items[Siege_Engine.SiegeInventoryIndex].to_weapon, skill);
                            targets.AddRange(result);
                ...
                foreach (Siege_Engine siege in Global.game_map.siege_engines.Values)
                        ...
                        Global.data_weapons[siege.item.Id].is_attack_staff())
                    {
                        ...
                        if (move_distance.IsSomething)
                        {
                            ...
                            targets.AddRange(result);
                            if (result.Any())
                                in_range_weapons.Add(Siege_Engine.SiegeInventoryIndex);
        ...
        private HashSet<int> battler_animation_ids(BattlerSpriteData battlerData, int distance)
        {
            ...
            var anim_set = TactileBattlerImage.animation_set(
                BattlerImageWrapper.animation_processor(battlerData, distance));
            if (anim_set != null)
            {
            ...
            else
            {
                if (actor.weapon != null)
                    foreach (int hit in Enumerable.Range(0, 2))
                        ...
                        {
                            result.UnionWith(BattlerImageWrapper.attack_animation_value(battlerData, crt == 0, distance, hit == 0));
                            result.UnionWith(BattlerImageWrapper.still_animation_value(battlerData, crt == 0, distance, hit == 0));
            ...
            result.UnionWith(BattlerImageWrapper.get_hit_animation_value(battlerData, false, distance));
            result.UnionWith(BattlerImageWrapper.get_hit_animation_value(battlerData, true, distance));
            result.UnionWith(BattlerImageWrapper.pre_battle_animation_value(battlerData, distance));
            return result;
        }
        ...
        {
            foreach (int id in BattlerImageWrapper.spell_attack_animation_value(actor.weapon_id, this.magic_attack, 1, true))
                yield return id;
            foreach (int id in BattlerImageWrapper.spell_attack_animation_value_bg1(actor.weapon_id, this.magic_attack, 1, true))
                yield return id;
            foreach (int id in BattlerImageWrapper.spell_attack_animation_value_bg2(actor.weapon_id, this.magic_attack, 1, true))
                yield return id;
            foreach (int id in BattlerImageWrapper.spell_attack_animation_value_fg(actor.weapon_id, this.magic_attack, 1, true))
                yield return id;
            foreach (int id in BattlerImageWrapper.spell_end_animation_value(actor.weapon_id, this.magic_attack, true))
                yield return id;
            foreach (int id in BattlerImageWrapper.spell_lifedrain_animation_value(actor.weapon_id, this.magic_attack, 1))
                yield return id;
            foreach (int id in BattlerImageWrapper.spell_lifedrain_end_animation_value(actor.weapon_id, this.magic_attack))
                yield return id;
        }
        ...
        {
            HashSet<int> result = new HashSet<int>();
            result.UnionWith(BattlerImageWrapper.dance_attack_animation_value(battlerData));
            result.UnionWith(BattlerImageWrapper.dance_still_animation_value(battlerData));
            result.UnionWith(BattlerImageWrapper.dance_return_animation_value(battlerData));
        ...
        private HashSet<int> dance_effect_animation_ids(BattlerSpriteData battlerData)
        {
            ...
            result.UnionWith(BattlerImageWrapper.refresh_animation_value(battlerData, ring_id));
            return result;
        }
        ...
        private HashSet<int> skill_animation_ids(BattlerSpriteData battlerData)
        {
            ...
            foreach (int animation_id in actor.all_skills.Select(x => Global.data_skills[x].Animation_Id))
                if (animation_id > -1)
                {
                    ...
                    List<int> anim = BattlerImageWrapper.correct_animation_value(id, battlerData);
                    if (anim.Count > 0)
                ...
                if (added_animations)
                {
                    ...
                    if (added_effects != null)
                        foreach (var added_effect in added_effects.Select(x => x.Item2))
                        {
                            ...
                            {
                                List<int> corrected = BattlerImageWrapper.correct_animation_value(anims[i], battlerData);
                                anims.RemoveAt(i);
