    ...
    class Battler_Sprite : Sprite
    {
        ...
        protected void initialize_palette()
        {
            ...
            else
            {
                ...
                {
                    List<int> anim = BattlerImageWrapper.attack_animation_value(Battler, false, 1, false);
                    if (anim.Count > 0)
        ...
        public static Color[] palette_data(string image_name, int class_id, int gender, string name)
        {
            ...
            if (Global.battlerPaletteData.ContainsKey(image_name))
            {
                ...
                // First, get the colors defined by Tactile.Recolor
                if (class_id > -1)
                {
                    ...
                    {
                        Color? target_color = Tactile.Recolor.get_color(class_id, gender, name, i);
                    ...
                    if (!recolors_found)
                        for (int i = 0; i < color_count; i++)
                        {
                            Color? target_color = Tactile.Recolor.get_color(class_id, gender % 2, name, i);
                            if (target_color != null)
                            {
                                if (palette.Count == i)
                                    palette.Add((Color)target_color);
                ...
                TactileLibrary.Palette.RecolorData recolorData;
                ...
                if (recolorData != null)
                {
                    var recolors = paletteData.GetRecolors(recolorData, Tactile.Recolor.RAMP_DEFAULT_OTHER_NAMES);
        ...
        public void idle_anim()
        {
            ...
            List<int> anim = BattlerImageWrapper.idle_animation_value(Battler, 1); // Distance? //Yeti
            if (anim.Count == 0)
            {
#if DEBUG
        ...
        public void avoid()
        {
            ...
            List<int> anim = BattlerImageWrapper.avoid_animation_value(Battler, 1); // Distance? //Yeti
            if (anim.Count == 0)
            {
                idle_anim();
            }
        ...
        public void avoid_return()
        {
            ...
            List<int> anim = BattlerImageWrapper.avoid_return_animation_value(Battler, 1); // Distance? //Yeti
            if (anim.Count == 0)
            {
                idle_anim();
            }
        ...
        protected void pre_battle_anim(int distance)
        {
            ...
            List<int> anim = BattlerImageWrapper.pre_battle_animation_value(Battler, distance);
            if (anim.Count == 0)
                idle_anim();
            else
            {
        ...
        public void attack(bool crit, int distance, bool no_damage, bool kill, bool hit)
        {
            ...
            List<int> anim = BattlerImageWrapper.attack_animation_value(Battler, crit, distance, hit);
            if (anim.Count == 0)
                idle_anim();
            else
            {
        ...
        public void hit_freeze(bool crit, int distance, bool no_damage, bool kill, bool hit)
        {
            ...
            List<int> anim = BattlerImageWrapper.still_animation_value(Battler, crit, distance, hit);
            if (anim.Count == 0)
                idle_anim();
            else
            {
        ...
        public void return_anim(bool crit, int distance, bool no_damage, bool kill, bool hit)
        {
            ...
            List<int> anim = BattlerImageWrapper.return_animation_value(Battler, crit, distance, hit, no_damage);
            if (anim.Count == 0)
                idle_anim();
            else
            {
        ...
        public void dance_attack()
        {
            ...
            List<int> anim = BattlerImageWrapper.dance_attack_animation_value(Battler);
            if (anim.Count == 0)
                idle_anim();
            else
            {
        ...
        public void dance_hit_freeze()
        {
            ...
            List<int> anim = BattlerImageWrapper.dance_still_animation_value(Battler);
            if (anim.Count == 0)
                idle_anim();
            else
            {
        ...
        public void dance_return_anim()
        {
            ...
            List<int> anim = BattlerImageWrapper.dance_return_animation_value(Battler);
            if (anim.Count == 0)
                idle_anim();
            else
            {
        ...
        public bool is_skill_time(bool crt)
        {
            ...
            int time_offset = 1;
            if (TactileBattlerImage.SKILL_ACTIVATION_FRAME.ContainsKey(Battler.ClassId))
                time_offset = Math.Max(1, TactileBattlerImage.SKILL_ACTIVATION_FRAME[Battler.ClassId][crt ? 1 : 0]);
        ...
        public void skill_animation(int distance)
        {
            ...
            if (id != null && id != -1)
            {
                ...
                List<int> anim = BattlerImageWrapper.correct_animation_value((int)id, Battler);
                if (anim.Count > 0)
                    if (anim[0] > -1)
                    {
        ...
        public void attack_spell(bool hit, bool crit, int distance)
        {
            ...
            List<int> anim;
            anim = BattlerImageWrapper.spell_attack_animation_value(
            ...
            anim = BattlerImageWrapper.spell_attack_animation_value_bg1(
                Battler.WeaponId, Battler.MagicAttack, distance, hit);
            if (anim.Count > 0)
            {
            ...
            anim = BattlerImageWrapper.spell_attack_animation_value_bg2(
                Battler.WeaponId, Battler.MagicAttack, distance, hit);
            if (anim.Count > 0)
            {
            ...
            anim = BattlerImageWrapper.spell_attack_animation_value_fg(
                Battler.WeaponId, Battler.MagicAttack, distance, hit);
            if (anim.Count > 0)
            {
        ...
        public void end_spell(bool hit, bool crit, int distance)
        {
            ...
            List<int> anim = BattlerImageWrapper.spell_end_animation_value(
        ...
        public void life_drain_spell_1(bool hit, bool crit, int distance)
        {
            ...
            List<int> anim = BattlerImageWrapper.spell_lifedrain_animation_value(
        ...
        public void life_drain_spell_2(bool hit, bool crit, int distance)
        {
            ...
            List<int> anim = BattlerImageWrapper.spell_lifedrain_end_animation_value(
        ...
        public void class_change_start()
        {
            ...
            Promotion_Data = TactileBattlerImage.PROMOTION_START;
            update_promotion();
            List<int> anim;
        ...
        public void class_change_end()
        {
            ...
            Promotion_Data = TactileBattlerImage.PROMOTION_END;
            update_promotion();
            List<int> anim;
            int anim_offset = Global.animation_group("Promotion");
            // Lightning effects
                ...
                if (added_effects != null)
                    foreach(var added_effect in added_effects)
                        if (added_effect.Item1 == Animation.anim_timer)
                        {
                            ...
                            {
                                List<int> corrected = BattlerImageWrapper.correct_animation_value(anims[i], Battler);
                                anims.RemoveAt(i);
        ...
        public float battler_stereo_offset()
        {
            ...
            //@Debug: this should be able to use lerp?
            float scale_percent =
                (scale.X - Global.BattleSceneConfig.BattlerMinScale) /
                (1 - Global.BattleSceneConfig.BattlerMinScale);
