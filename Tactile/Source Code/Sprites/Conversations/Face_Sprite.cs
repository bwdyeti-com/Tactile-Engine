    ...
    class Face_Sprite : Sprite
    {
        ...
        internal string filename
        {
            set
            {
                ...
                else if (Global.content_exists(@"Graphics/Faces/" + name.Split(Global.ActorConfig.ActorNameDelimiter)[0]))
                    Filename = name.Split(Global.ActorConfig.ActorNameDelimiter)[0];
                // Default face exists
        ...
        protected TactileLibrary.RectangleExclusion frame_exclusion()
        {
            return new TactileLibrary.RectangleExclusion(this.src_rect, frame_rects());
        }
        ...
        public void recolor_country(string country)
        {
            ...
            if (Face_Recolor.COUNTRY_COLORS.ContainsKey(country) && texture != null)
            {
                ...
                foreach (KeyValuePair<Tactile.Face_Color_Keys, Color> pair in Tactile.Face_Recolor.COUNTRY_COLORS[country])
                {
                    recolors.Add(Tactile.Face_Recolor.COUNTRY_COLORS["Default"][pair.Key], pair.Value);
                }
        ...
        public TactileLibrary.Face_Data face_data
        {
            get
            {
        ...
        public static TactileLibrary.Face_Data get_face_data(string filename)
        {
            string name = filename.Split(Global.ActorConfig.BuildNameDelimiter)[0];
            if (Global.face_data != null && Global.face_data.ContainsKey(name)) //FaceData //Debug
                return Global.face_data[name];
            return new TactileLibrary.Face_Data();
        }
