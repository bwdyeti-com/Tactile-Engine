...
{
    class Pathfinder<T> where T : IEquatable<T>
    {
        ...
        public Pathfinder(IMovementMap<T> map)
        {
            Map = map;
        }
        ...
        public List<T> get_route(T loc, T target_loc, int mov)
        {
                ...
                else
                {
                    ...
                    foreach (T test_loc in check_locs)
                    {
                        ...
                        if (Map.InvalidLocation(test_loc, target_loc, restrict_to_map))
                            continue;
                        check_tile(test_loc, last_added, mov, target_loc, open_list, closed_list, use_euclidean_distance: use_euclidean_distance);
                    }
                }
            }
            RouteDistance = 0;
            if (route_found)
            {
                RouteDistance = closed_list.get_g(last_added) / 10;
        ...
        public List<T> get_reverse_route(T loc, T target_loc, int mov)
        {
            ...
            // Start pathfinding
            bool route_found = false;
            open_list.add_initial_item(loc, Map.TileCost(loc), Map.Distance(loc, target_loc));
            int last_added = -1;
            for (; ; )
            {
                if (open_list.size <= 0)
                ...
                else
                {
                    ...
                    foreach (T test_loc in check_locs)
                    {
                        ...
                        if (Map.InvalidLocation(test_loc, target_loc, restrict_to_map))
                            continue;
                        check_tile(test_loc, last_added, mov, target_loc, open_list, closed_list);
                    }
                }
            }
            RouteDistance = 0;
        ...
        public Maybe<int> get_distance(T loc, T target_loc, int mov)
        {
            ...
            if (route_found)
            {
                return closed_list.get_g(last_added) / 10;
            }
            ...
            OpenList<T> open_list, ClosedListRoute<T> closed_list, bool dijkstras = false, bool use_euclidean_distance = false)
        {
            ...
            if (pass)
            {
                int move_cost = Map.TileCost(loc);
                int g = move_cost + closed_list.get_g(parent);
                if (mov < 0 || g <= mov * 10)
                {
                    int heuristic = Map.HeuristicPenalty(loc);
                    if (!dijkstras)
