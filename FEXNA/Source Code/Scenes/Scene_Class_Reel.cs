...
namespace FEXNA
{
    ...
    class Scene_Class_Reel : Scene_Base
    {
        ...
        protected FE_Text Name, Name_Bg;
        protected Class_Reel_Stat_Bars[] Stat_Bars = new Class_Reel_Stat_Bars[7];
        protected FE_Text[] Stat_Labels = new FE_Text[7];
        protected FE_Text[] Stat_Values = new FE_Text[7];
        ...
        protected void initialize_images()
        {
            ...
            Name_Bg.Font = "FE7_Reel2";
            Name_Bg.texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Reel2");
            ...
            {
                Stat_Values[i] = new FE_Text_Int();
                Stat_Values[i].loc = new Vector2(48, 8 + 16 * i);
                Stat_Values[i].Font = "FE7_Text";
                Stat_Values[i].texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_White");
        ...
        protected void add_letter()
        {
            ...
            if (Letters.Count == 1)
            {
                int width = Font_Data.text_width(Unit.actor.class_name, "FE7_Reel");
                Letters[0].loc.X = Config.WINDOW_WIDTH / 2 - width / 2;
            }
            else
            {
                int x = Font_Data.text_width(Letters[Letters.Count - 2].text, "FE7_Reel");
        ...
        protected void load_class(Class_Reel_Data data)
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                int j = i + (Class_Reel_Data.SKIP_LCK && i >= (int)FEXNA.Stat_Labels.Lck ? 1 : 0);
        ...
        protected void set_stats()
        {
            ...
            Name.offset.X = Font_Data.text_width(Name.text, Name.Font);
            ...
            RenderTarget2D final_render, RenderTarget2D temp_render, RenderTarget2D effect_render)
        {
            ...
            if (Message != null)
            {
                ...
                foreach (FE_Text value in Stat_Values)
