...
namespace FEXNA
{
    class Arena_Background : Sprite
    {
        ...
        public override void update()
        {
            ...
            base.update();
            Bg_Timer = (Bg_Timer + 1) % Constants.BattleScene.ARENA_BG_TIME;
            src_rect = new Rectangle(
                0,
                160 * (Bg_Timer / (Constants.BattleScene.ARENA_BG_TIME /
                    Constants.BattleScene.ARENA_BG_FRAMES)),
                Config.WINDOW_WIDTH,
                160);
        }
        ...
        public override void draw(SpriteBatch sprite_batch, Vector2 draw_offset = default(Vector2))
        {
            if (texture != null)
                if (visible)
                {
                    int y = 160 * (Bg_Timer / (Constants.BattleScene.ARENA_BG_TIME /
                        Constants.BattleScene.ARENA_BG_FRAMES));
                    int oy = 0;
