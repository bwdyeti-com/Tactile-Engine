...
namespace FEXNA
{
    public class GameLoop: ISaveCallbacker
    {
        ...
        {
            Thread.Sleep(10);
            FEXNA_Library.Maybe<bool> result = FEXNA_Library.Maybe<bool>.Nothing;
#if WINDOWS || MONOMAC
        ...
        private void InitializeTextGlow()
        {
            ...
            {
                Texture2D texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_" + color);
        ...
        private void UpdateTextGlow(int ticks = 1)
        {
            ...
            if (Enumerable.Range(1, ticks).Any(x => (glowTimer + x) % 4 == 0))
            {
                ...
                if (currentIndex != oldIndex)
                {
                    foreach (string color in TEXT_GLOW.Keys)
                    {
                        Texture2D text = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_" + color);
