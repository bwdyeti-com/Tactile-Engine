...
namespace FEXNA
{
    public partial class Event_Processor
    {
        ...
        private FEXNA_Library.Event_Data event_data;
        ...
        private FEXNA_Library.Event_Control command { get { return event_data.data[Index]; } }
        ...
        public Event_Processor(FEXNA_Library.Event_Data data)
        {
            Id = -1;
            event_data = data;
            Key = "";
        ...
        private void initialize()
        {
            ...
            {
                FEXNA_Library.Event_Control control = event_data.data[i];
                // Skip comments
                if (control.Key == 102)
                {
                    Indent.Add(indent);
        ...
        {
            if (!FEXNA_Config.Event_Control_Names.CONTROLS.ContainsKey(key))
                throw new KeyNotFoundException(string.Format("No event control name for event code {0}", key));
            return FEXNA_Config.Event_Control_Names.CONTROLS[key];
        }
#endif
    }
}
