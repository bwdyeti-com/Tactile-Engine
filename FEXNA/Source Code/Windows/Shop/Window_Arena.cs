...
namespace FEXNA
{
    ...
    class Window_Arena : Window_Business
    {
        ...
        private void create_opponent_window()
        {
            ...
            {
                Labels[i] = new FE_Text();
                Labels[i].loc = Window.loc + new Vector2(8 + 56 * (i % 2), 8 + 16 * (i / 2));
                Labels[i].Font = "FE7_Text";
                Labels[i].texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_White");
            }
            Labels[4] = new FE_Text_Int();
            Labels[4].loc = Window.loc + new Vector2(8 + 40, 8);
            Labels[4].Font = "FE7_Text";
            Labels[4].texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_Blue");
            Labels[0].text = "Lv";
        ...
        private float combat_odds(Game_Unit unit, Game_Unit opponent)
        {
            ...
            /*float[] use = Combat.damage_per_round(unit, opponent, new List<int> { Actual_Distance }, null, true, true);
            // Divides hp by damage/round to get rounds until kill
            if (use[0] > 0)
                use[0] = opponent.actor.maxhp / use[0];
            else
                use[0] = 255;
            if (use[1] > 0)
                use[1] = unit.actor.maxhp / use[1];
            else
                use[1] = 255;
            float old_odds = use[0] / (use[0] + use[1]);*/
