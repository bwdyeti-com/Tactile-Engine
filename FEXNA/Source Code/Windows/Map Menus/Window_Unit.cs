...
namespace FEXNA.Windows.Map
{
    ...
    internal class Window_Unit : Map_Window_Base
    {
        ...
        protected FE_Text Page_Number, Max_Page_Number, Page_Number_Slash;
        ...
        protected void initialize_sprites()
        {
            ...
            // Header
            Name_Header = new FE_Text();
            Name_Header.loc = new Vector2(40 + Unit_Scissor_Rect.X, Unit_Scissor_Rect.Y - 16);
            Name_Header.Font = "FE7_Text";
            Name_Header.texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_White");
            ...
            // Page Numbers
            Page_Number = new FE_Text_Int();
            Page_Number.loc = new Vector2(Config.WINDOW_WIDTH - 28, 8);
            Page_Number.Font = "FE7_Text";
            Page_Number.texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_Blue");
            Page_Number.stereoscopic = Config.UNIT_SORT_DEPTH;
            Page_Number_Slash = new FE_Text();
            Page_Number_Slash.loc = new Vector2(Config.WINDOW_WIDTH - 28, 8);
            Page_Number_Slash.Font = "FE7_Text";
            Page_Number_Slash.texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_White");
            Page_Number_Slash.text = "/";
            Page_Number_Slash.stereoscopic = Config.UNIT_SORT_DEPTH;
            Max_Page_Number = new FE_Text_Int();
            Max_Page_Number.loc = new Vector2(Config.WINDOW_WIDTH - 12, 8);
            Max_Page_Number.Font = "FE7_Text";
            Max_Page_Number.texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_Blue");
        ...
        protected virtual void set_headers()
        {
            for (int i = 0; i < Pages; i++)
            {
                ...
                foreach (KeyValuePair<int, string> header in header_page)
                {
                    ...
                    {
                        Headers.Add(new FE_Text());
                        ((FE_Text)Headers[Headers.Count - 1]).loc = new Vector2(header.Key + i * data_width + Header_Scissor_Rect.X, Header_Scissor_Rect.Y);
                        ((FE_Text)Headers[Headers.Count - 1]).Font = "FE7_Text";
                        ((FE_Text)Headers[Headers.Count - 1]).texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_White");
                        ((FE_Text)Headers[Headers.Count - 1]).text = header.Value;
                    }
        ...
        protected void refresh_unit_name_and_sprite_texture(int index)
        {
            ...
            // Name
            if (Preparations)
                Names[index].texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_" + prep_name_color(actor));
            else
                Names[index].texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_White");
        }
            ...
            switch ((Unit_Sort)new_sort)
            {
                ...
                case Unit_Sort.Equip:
                    ...
                    if (Global.data_weapons.ContainsKey(weapon_id_a) && Global.data_weapons.ContainsKey(weapon_id_b))
                    {
                        ...
                        if ((weapon_a.Rank == FEXNA_Library.Weapon_Ranks.None && weapon_a.is_prf) ||
                                (weapon_b.Rank == FEXNA_Library.Weapon_Ranks.None && weapon_b.is_prf))
                ...
                case Unit_Sort.Staff:
                    FEXNA_Library.WeaponType type = Global.weapon_types[new_sort - (int)Unit_Sort.Sword + 1];
        ...
        protected virtual void refresh_sort_images()
        {
            switch ((Unit_Sort)sort)
            {
                ...
                case Unit_Sort.Staff:
                    ...
                    break;
                default:
                    FE_Text sort_label_text = new FE_Text();
                    sort_label_text.Font = "FE7_Text";
                    sort_label_text.texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_White");
