...
namespace FEXNA.Windows.Target
{
    ...
    class Window_Target_Staff : Window_Target_Unit
    {
        ...
        protected List<FE_Text> StatLabels;
        protected List<FE_Text> Stats;
        ...
        public static Staff_Target_Mode target_mode(FEXNA_Library.Data_Weapon staff)
        {
        ...
        protected virtual void initialize_stat_labels()
        {
            ...
            if (Mode == Staff_Target_Mode.Heal)
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    StatLabels.Add(new FE_Text());
                    StatLabels[i].offset = new Vector2(-8, -(24 + (i * 16)));
                    StatLabels[i].Font = "FE7_Text";
                    StatLabels[i].texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_Yellow");
                }
                StatLabels[0].Font = "FE7_TextL";
                ...
                if (Mode == Staff_Target_Mode.Barrier)
                {
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        Stats[i].texture = Global.Content.Load<Texture2D>(
                            @"Graphics/Fonts/FE7_Text_" + (i % 2 == 0 ? "Blue" : "Yellow"));
                    }
                    Stats[1].text = "-";
                    // Res
                    Stats[0].text = (actor2.stat(Stat_Labels.Res) +
                        target.temporary_stat_buff(FEXNA_Library.Buffs.Res)).ToString();
                    // New Res
        ...
        protected override void refresh()
        {
            if (Mode != Staff_Target_Mode.Torch)
            {
                ...
                foreach (FE_Text label in StatLabels)
                    label.loc = Loc;
