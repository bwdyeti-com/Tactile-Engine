...
namespace FEXNA.Windows.Target
{
    class Window_Target_Sacrifice : Window_Target_Staff
    {
        ...
        protected override void initialize_images()
        {
            ...
            // Names
            Unit_Name = new FE_Text();
            Unit_Name.Font = "FE7_Text";
            Unit_Name.texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_White");
        ...
        protected override void set_images()
        {
            if (Mode != Staff_Target_Mode.Torch)
            {
                ...
                if (Mode == Staff_Target_Mode.Heal)
                {
                    ...
                    for (int i = 5; i < 10; i++)
                    {
                        Stats.Add(i % 2 == 1 ? new FE_Text_Int() : new FE_Text());
                        Stats[i].offset = new Vector2(-(48 + (((i - 5) / 2) * 24)), -72);
                        Stats[i].Font = "FE7_Text";
                        Stats[i].texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_" + (i % 2 == 1 ? "Blue" : "White"));
                    }
